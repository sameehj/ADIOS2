                                  ...
                                  void *DP_TimestepInfo)
{
    ...
    {
        Svcs->verbose(RS_Stream->CP_Stream, DPTraceVerbose,
                      "Block address is %p, with a key of %.11s, and access key size %zu.\n", Info->Block,
                      Info->rkey, Info->rkey_size);
    }
    else
    {
        Svcs->verbose(RS_Stream->CP_Stream, DPCriticalVerbose,
...
static int UcxWaitForCompletion(CP_Services Svcs, void *Handle_v)
{
    ...
    } else { // UCS_OK request completed immediately. Call ucx callback.
        status = UCS_OK;
    }
                           ...
                           struct _SstParams *Params)
{
    ...
    /* The Ucx DP priority needs to be dynamic */
    //atom_t UCX_DP_PRIORITY = attr_atom_from_string("UCX_DP_PRIORITY");
    //Svcs->verbose(CP_Stream, DPPerStepVerbose, "priority value: %d\n", ux_dp_priority);
